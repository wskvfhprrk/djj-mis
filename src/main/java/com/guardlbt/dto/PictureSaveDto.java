...
public class PictureSaveDto {
    ...
    @ApiModelProperty(value ="图片访问路径",example = "")
    @NotBlank(message = "图片访问路径不能为空值")
    private String url;
    ...
    @ApiModelProperty(value ="是否有缩略图",example = "")
    @NotNull(message = "是否有缩略图不能为空值")
    private Integer isThumbImage;

    @ApiModelProperty(value ="状态（0:弃用，1:在用）",example = "")
    @NotNull(message = "状态（0:弃用，1:在用）不能为空值")
