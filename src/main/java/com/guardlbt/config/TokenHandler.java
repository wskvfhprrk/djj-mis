...
public class TokenHandler implements HandlerInterceptor {
    ...
    private List<String> noCertification;
    @Value("${token_time:30}")
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        String path = servletPath + "@" + method.toLowerCase();
        //无需认证的——以竖线隔开
        for (String s : noCertification.get(0).replace(" ", "").split("-/")) {
            boolean r = antPathMatcher.match("/" + s, path);
            if (r) {
                //如果无需要验证
                return true;
            }
        ...
        boolean r = false;
        for (String s : oauthByToken) {
            s = s.replaceAll("\"", "");
            r = antPathMatcher.match(s, path);
            if (r) {
                //如果无需要验证
                return true;
            }
