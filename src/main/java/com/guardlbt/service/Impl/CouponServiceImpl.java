...
public class CouponServiceImpl implements CouponService {
    ...
    @Override
    @Transactional
    public Result insert(CouponSaveDto dto, HttpServletRequest request) {
        Coupon coupon=new Coupon();
        BeanUtils.copyProperties(dto,coupon);
        ...
        try{
            ...
            dao.insert(coupon);
            couponHistoryServiceInsert(coupon,request);
            return Result.ok();
        }catch (Exception e){
            e.printStackTrace();
     ...
     * @param coupon
     */
    private void couponHistoryServiceInsert(Coupon coupon, HttpServletRequest request) {
        User user = tokenUtil.getUserByToken(TokenUtil.getToken(request));
        CouponHistorySaveDto dto=new CouponHistorySaveDto();
        dto.setCouponNo(coupon.getCouponId());
        dto.setRemark("新建电子券");
        dto.setCreateTime(new Date());
        dto.setToName("no");
        dto.setFromName(user.getUserId()+"+"+user.getRealName());
        couponHistoryService.insert(dto);
    }
